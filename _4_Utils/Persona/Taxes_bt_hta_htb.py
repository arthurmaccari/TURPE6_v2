...
def compute_turpe(client):
    ...
    def get_CG_part(contract):
        ...
        if contract.tensionDomain[:-1] == "htb":
            return params[contract.tensionDomain]["CG"]["a"]
    ...
    def get_CC_part(contract):
        nonlocal params
        if contract.tensionDomain[:-1] == "htb":
            if contract.ownsMeter:
                return params[contract.tensionDomain]["CC"]["compteur_utilisateur"]
            return params[contract.tensionDomain]["CC"]["compteur_gestionnaire"]
    ...
    def compute_CS_part(contract):
        ...
        if contract.tensionDomain == "htb3":
            ...
            return sum(contract.elecCons)*c

        b = params[contract.tensionDomain]["CS"][contract.tariffOption]["b"]
        if  contract.tariffOption == "CU4_autoprod_collective" or \
            contract.tariffOption == "MU4_autoprod_collective" or \
            contract.tariffOption == "LU4_autoprod_collective":
...
for tensionDomain in ["btinf36", "btsup36", "hta", "htb1", "htb2", "htb3"]:
    
    contT = ["CARD", "contrat_unique"]
    if tensionDomain == "hta":
        consT = ["consommateur", "autoprod_indiv_avec_inj", "autoprod_indiv_sans_inj"]
    elif tensionDomain[:-1] == "htb":
        consT = ["consommateur"]
        contT = [""]
    else:
        consT = ["consommateur", "autoprod_indiv_avec_inj", "autoprod_indiv_sans_inj", "autoprod_collectif"]

    for consumerType in consT:
        for contractType in contT:
            ...
            if tensionDomain == "btinf36":
                ...
                if consumerType == "consommateur":
                    elecCons = [6640+106,7760+45,1680+147,2040+67] # for tests + AM to change for 2 distinct variables (alloproduit and autoproduit)
                    to = ["CU4","LU4"]
                else:
                    elecCons = [6640+106,7760+45,1680+147,2040+67,0,0,0,0] # for tests + AM to change for 2 distinct variables (alloproduit and autoproduit)
                ...
                powConSub = [37,38,39,40,41] # for tests + not the good domain
                elecCons = [6640+106,7760+45,1680+147,2040+67,0] # for tests
                if tensionDomain == "htb3":
                    to = [""]
                else:
                    to = ["CU","MU","LU"]

            for tariffOption in to:
                if tensionDomain[:-1] == "htb":
                    for ownsMeter in [True, False]:
                        contract = Contract_pm(None, None, tensionDomain, powConSub, contractType, tariffOption, elecCons, None, None, True, ownsMeter)
                        ...
                        print("hasMeter : ", True)
                        print("ownsMeter : ", ownsMeter)
                        print(compute_turpe(client))
                        print("-------")
                else:
